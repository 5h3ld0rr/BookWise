...
namespace BookWise
{
    public static class CommonData
    {
        public static class Stats
        {
            ...
            {
                string query = "SELECT (SELECT COUNT(*) FROM users) AS TotalUsers,(SELECT COALESCE(SUM(available_books),0) FROM books) AS AvailableBooks, (SELECT COUNT(*) FROM book_transactions WHERE ISNULL(return_date)) AS BorrowedBooks";
